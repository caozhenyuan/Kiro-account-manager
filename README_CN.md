...
### v1.7.9 (2026-8-28) — 流式输出重复文本/前缀加固 + 原始流诊断日志

#### 🐛 流式事件双重 emit 加固

- **修复**: `codeEvent` 分支由独立 `if` 改为与 `assistantResponseEvent` 互斥的 `else if`。此前同一帧若同时携带两个字段（或判断误命中），正文 `content` 会被 emit 两次，可能导致重复文本 / 每段重复前缀（如观察到的每行前缀重复某单词）。改互斥后正文只处理一次

#### 🔍 诊断

- **新增**: 环境变量 `KIRO_STREAM_DUMP=1`，开启后逐帧打印原始 event 的 type + content 片段，用于定性输出错位来源（模型原始输出即如此 vs 代理重复 emit）。默认关闭，不影响正常运行
